...
public class BootstrapSnapshotRepository {
    private static final String SNAPSHOT_CTE = """
            WITH historical AS (
                ...
                SELECT r.entity_type, r.entity_id, r.version, r.deleted,
                       COALESCE(master.payload_json, r.payload_json) AS payload_json,
                       r.updated_at
                FROM sync_records r
                LEFT JOIN sync_master_payloads master
                  ON master.entity_type = r.entity_type
                 AND master.entity_id = r.entity_id
    ...
    private static final String DEPENDENCY_ORDER = """
            CASE entity_type
              ...
              WHEN 'PRICE_LIST' THEN 95 WHEN 'PRODUCT' THEN 100 WHEN 'CLIENT' THEN 200
              WHEN 'ORDER_SHEET' THEN 300 WHEN 'ORDER' THEN 310 WHEN 'ORDER_ITEM' THEN 320
              WHEN 'QUOTE' THEN 400 WHEN 'QUOTE_ITEM' THEN 410 WHEN 'PREPARATION' THEN 500
