...
SELECT 'COMPANY' entity_type, id entity_id,
       JSON_OBJECT('id', id, 'nombre', nombre, 'ruc', ruc, 'telefono', telefono,
            'direccion', direccion, 'estado', estado, 'actualizado_en', CAST(updated_at AS CHAR)) payload_json
FROM empresas
UNION ALL
SELECT 'BRAND', id,
       JSON_OBJECT('id', id, 'empresa_id', empresa_id, 'nombre', nombre, 'estado', estado,
            'actualizado_en', CAST(updated_at AS CHAR))
FROM marcas
UNION ALL
SELECT 'CATEGORY', id,
       JSON_OBJECT('id', id, 'categoria_padre_id', categoria_padre_id, 'nombre', nombre,
            'descripcion', descripcion, 'estado', estado, 'actualizado_en', CAST(updated_at AS CHAR))
FROM categorias
UNION ALL
SELECT 'BRAND_CATEGORY', id,
       JSON_OBJECT('id', id, 'marca_id', marca_id, 'categoria_id', categoria_id,
            'estado', estado, 'actualizado_en', CAST(updated_at AS CHAR))
FROM marca_categorias
UNION ALL
SELECT 'MEASUREMENT_UNIT', id,
       JSON_OBJECT('id', id, 'codigo', codigo, 'nombre', nombre, 'simbolo', simbolo,
            'magnitud', magnitud, 'factor_a_base', factor_a_base, 'decimales', decimales, 'estado', estado)
FROM unidades_medida
UNION ALL
SELECT 'CATEGORY_ATTRIBUTE', id,
       JSON_OBJECT('id', id, 'categoria_id', categoria_id, 'nombre', nombre, 'clave', clave,
            ...
            'orden', orden, 'estado', estado, 'actualizado_en', CAST(updated_at AS CHAR))
FROM categoria_atributos
UNION ALL
SELECT 'CATEGORY_ATTRIBUTE_OPTION', id,
       JSON_OBJECT('id', id, 'categoria_atributo_id', categoria_atributo_id, 'etiqueta', etiqueta,
            'codigo', codigo, 'orden', orden, 'estado', estado)
FROM categoria_atributo_opciones
UNION ALL
SELECT 'CATEGORY_ATTRIBUTE_UNIT', id,
       JSON_OBJECT('id', id, 'categoria_atributo_id', categoria_atributo_id,
...
SELECT 'LEGACY_ATTRIBUTE_DEFINITION', id,
       JSON_OBJECT('id', id, 'categoria_id', categoria_id, 'nombre', nombre,
            'tipo', tipo, 'es_variante', es_variante)
FROM atributos_def
UNION ALL
SELECT 'PRICE_LIST', id,
       JSON_OBJECT('id', id, 'nombre', nombre, 'moneda', moneda, 'incluye_igv', incluye_igv,
